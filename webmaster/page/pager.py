...
class Pagination(object):
    ...
    def page_html(self):
        ...
        page_html_list = ['<nav aria-label="Page navigation">','<ul class="pagination">']
        # {source:[2,], status:[2], gender:[2],consultant:[1],page:[1]}
        self.params['page'] = 1
        first_page = '<li><a href="%s?%s" aria-label="Previous"><span aria-hidden="true">首页</span></a></li>' % (self.base_url,self.params.urlencode(),)
        page_html_list.append(first_page)
        # 上一页
        if self.current_page-1 > 0:
            self.params['page'] = self.current_page-1
            previous = '<li><a href="%s?%s" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'% (self.base_url,self.params.urlencode(),)
        ...
        if self.current_page+1 <= self.max_page_num:
            self.params["page"] = self.current_page+1
            next = '<li><a href="%s?%s" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>' % (self.base_url,self.params.urlencode())
            page_html_list.append(next)
        self.params['page'] = self.max_page_num
        last_page = '<li><a href="%s?%s" aria-label="Next"><span aria-hidden="true">尾页</span></a></li></ul></nav>' % (self.base_url, self.params.urlencode(),)
